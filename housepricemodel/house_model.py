...
def model(data):
    ...
    learn_rate = 0.025  # 学习率
    epoch = 500  # 训练轮数
    with tf.name_scope("Input"):
        # 参数占位
        X_data = tf.placeholder(tf.float32, shape=x_data.shape,name="input_x")
        Y_data = tf.placeholder(tf.float32, shape=y_data.shape,name="input_y")
    with tf.name_scope("Weight"):
        # 权重变量
        w = tf.get_variable("weight", (x_data.shape[1], 1), initializer=tf.constant_initializer())
    with tf.name_scope("Hypothesis"):
        # 前置运算
        y_pre = tf.matmul(X_data, w,name="y_prediction")
    with tf.name_scope("Loss"):
        # 损失函数,转置矩阵a
        loss_op = 1 / (2 * len(x_data)) * tf.matmul((y_pre - Y_data), (y_pre - Y_data), transpose_a=True)
    with tf.name_scope("Optimizer"):
        # 梯度下降优化
        train_op = tf.train.GradientDescentOptimizer(learn_rate).minimize(loss_op)
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        fileWriter=tf.summary.FileWriter("./summary/line-0",sess.graph)
